...
import { Plus, Edit2, Trash2, Search, X, BookOpen, ExternalLink, Save } from 'lucide-react';
import { useAdminStore } from '@/store/useAdminStore';
import { toast } from 'sonner';

export default function JurnalManager() {
  const { jurnals, addJurnal, updateJurnal, deleteJurnal } = useAdminStore();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateJurnal(editingId, form);
      toast.success('Referensi Jurnal berhasil diperbarui!');
    } else {
      addJurnal({ ...form, id: `jurnal-${Date.now()}` });
      toast.success('Referensi Jurnal berhasil ditambahkan!');
    }
  ...
  const handleDelete = (id: string, title: string) => {
    if (window.confirm(`Yakin ingin menghapus jurnal "${title}"?`)) {
      deleteJurnal(id);
      toast.success('Referensi Jurnal berhasil dihapus!');
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
        <div>
          <h1 className="text-2xl font-bold text-navy flex items-center gap-2">
            ...
            Manajemen Referensi Jurnal
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Kelola daftar tautan publikasi ilmiah dan jurnal sosiologi untuk referensi mahasiswa.
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1">Judul Jurnal / Artikel <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  required
                  value={form.title}
                  ...
                  placeholder="Contoh: Jurnal Sosiologi Nusantara"
                />
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-slate-700 mb-1">Tautan Publikasi (URL) <span className="text-red-500">*</span></label>
                <input
                  type="url"
                  required
                  value={form.link}
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-amber/20 focus:border-amber outline-none transition-all"
                  placeholder="Jelaskan secara singkat topik jurnal ini..."
                />
              </div>
            </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
          ...
          <div className="text-sm text-slate-500 font-medium">
            Total: {filteredJurnals.length} Jurnal
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-100/50 text-slate-600 text-sm">
                <th className="px-6 py-4 font-semibold w-1/4">Judul Jurnal</th>
                <th className="px-6 py-4 font-semibold w-2/4">Deskripsi</th>
                <th className="px-6 py-4 font-semibold w-1/4">Tautan Tuju</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredJurnals.length > 0 ? (
                filteredJurnals.map((jurnal) => (
                  <tr key={jurnal.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="font-medium text-navy">{jurnal.title}</div>
                    </td>
                    <td className="px-6 py-4">
                      <div className="text-sm text-slate-600 line-clamp-2">{jurnal.description}</div>
                    </td>
                    <td className="px-6 py-4">
                      <a href={jurnal.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-blue-600 hover:text-blue-800 text-sm bg-blue-50 px-2 py-1 rounded-md transition-colors">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 transition-opacity">
                        ...
                        <button
                          onClick={() => handleDelete(jurnal.id, jurnal.title)}
